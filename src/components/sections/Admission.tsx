...
import { Calendar03Icon, PauseIcon, PlayIcon } from "@hugeicons/core-free-icons";
 ...
 * pointer is over the band or focus is inside it; it stops when the reader
 * drives the track themselves; it never starts under prefers-reduced-motion;
 * and there is a pause button next to the dots.
 *
 * The button is not decoration. Hover and DOM focus were the only two things
 * that paused this, and a phone reader has neither: touch fires no mouseenter
 * until a tap, and VoiceOver and TalkBack move a virtual cursor rather than
 * focus. On the device this site is mostly read on there was no way to stop a
 * 6-second auto-advance at all, which is WCAG 2.2.2 at Level A. "Tap the track
 * and it stops" was true and completely undiscoverable.
 *
 * Driving the track and pressing pause therefore land in the same state rather
 * than two competing ones, so the button can always undo whatever stopped it.
...
export function Admission() {
  ...
  /** Stopped: by the pause button, or by the reader driving the track. */
  ...
  useEffect(() => {
    if (paused || held || admissions.length < 2) return;
    ...
    >
      <Reveal>
        <div
          ...
          onWheel={(e) => {
            if (Math.abs(e.deltaX) > Math.abs(e.deltaY)) setPaused(true);
          }}
          onKeyDown={(e) => {
            if (["ArrowLeft", "ArrowRight", "Home", "End"].includes(e.key)) setPaused(true);
          }}
        >
        ...
        {admissions.length > 1 ? (
          <div className="admit-dots">
            ...
            <button
              type="button"
              className="admit-play"
              aria-label={paused ? "Play the batches" : "Pause the batches"}
              onClick={() => setPaused((v) => !v)}
            >
              <Icon icon={paused ? PlayIcon : PauseIcon} size={14} />
            </button>
          </div>
        ) : null}
      </Reveal>
