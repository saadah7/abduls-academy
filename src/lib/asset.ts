 ...
 *
 * GitHub Pages serves this as a project site under /abduls-academy/, and a
 * static export leaves next/image's src verbatim, so root-relative paths would
 * point at the wrong host root. NEXT_PUBLIC_BASE_PATH is inlined at build
 * time; it is empty on Vercel, which serves from its own domain root, and
